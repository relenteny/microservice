...
function execute_chartmuseum_delete() {
  ...
  if [[ "${line}" != "-" ]]; then
    ...
    curl -XDELETE ${helm_repository}/api/charts/${helm_chart_name}/${version} >/dev/null
  fi
}

function delete_chart_from_chartmuseum() {
  local semver=${helm_chart_semver}
  for line in $(curl -s ${helm_repository}/index.yaml | grep "charts/${helm_chart_name}-${semver}+"); do
...
function publish_chart() {
  ...
  status_code=$(curl -n -s -w "%{http_code}" ${helm_repository}/index.yaml -o /dev/null)
  if [[ "${status_code}" == "200" ]]; then
    ...
    if [[ "${deployment_type}" == "local" ]]; then
      curl --data-binary "@${packaged_chart_file}" ${helm_repository}/api/charts >/dev/null
    else
      curl -n -X PUT ${helm_repository}/${packaged_chart_file} -T "${packaged_chart_file}" >/dev/null
    fi
  else
    echo " "
